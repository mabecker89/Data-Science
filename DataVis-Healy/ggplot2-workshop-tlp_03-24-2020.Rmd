...
Aesthetic mapping: link variables in data to graphical properities in the geometry. 

+ E.g. this variable belogs to the X axis, this one the Y, this one is colour, etc.
...
- Categories -> Colour
- Number -> Position
Imply a specific interpretation of values; discrete, continuous, date, etc.
...
Small multiples -> allows you to look at small subsets of your data in separate plots.
...
ggplot(data = faithful, 
       mapping = aes(x = eruptions, y = waiting)) + # which columns to use for x and y
  geom_point() # the geometry.

# Data and mapping can be given both as global (in ggplot()) or per layer
ggplot() + 
  geom_point(mapping = aes(x = eruptions, y = waiting),
             data = faithful) # data is specific to this layer.
...
```{r}
ggplot(data = faithful) + 
  geom_point(mapping = aes(x = eruptions, y = waiting, colour = eruptions < 4 & waiting > 75)) # the colour mapping is an expression.

# Suddenly we get a 'guide' aka a legend.
...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting),
             # Set to a value. Outside of aes() call. Data does not dictate it, we just want it cause it looks nice :)
             colour = 'steelblue', size = 3)
```

Lots of different types of geoms. Their mapping requirements differ.

Some geoms only need a single mapping and will calculate the rest for you - e.g. geom_histogram(). 
...
# Statistics is helping us calculate what needs to be on the y axis. 
...
# the order of the addition of layers defines the order of layers in the plot.
...
```{r}
ggplot(faithful) + 
  geom_histogram(aes(x = eruptions, fill = eruptions > 3), color = "black")
```
...
# 'stacked' is the default (ie stacked) vs 'identity' (behind one another) vs 'dodge' (beside each other)
...
ggplot(faithful) + 
  geom_point(aes(x = eruptions, y = waiting)) +
  geom_abline(slope = -40, intercept = 200) # wingin' it.
```
...
ggplot(mpg_counted) + 
  geom_bar(aes(x = class, y = count), stat = 'identity') # need to map y now. 

# Or just use geom_col() shorcut (essentially geom_bar() with identity)
...
ggplot(mpg) + 
  geom_bar(aes(x = class, y = after_stat(100 * count / sum(count)))) # access `count` in this way.
...
In help files, look for 'Computed Variables" 
...
ggplot(mpg) +
  stat_count(aes(x = class)) # These are equivalent. Default geom for stat is bar. 
  
ggplot(mpg) +
  stat_summary(aes(x = class, y = hwy), fun = "median", colour = "red")
...
ggplot(mpg) + 
  ...
  stat_summary(aes(x = class, y = hwy), fun = "mean", color = "red", geom = "point", size = 3)
  
# Use stat_summary() to calculate info about your data.
...
Now you can take control by adding one explicitly. All scales follow the same naming 
conventions.
...
# While function name is predicable (e.g. scale_<aes>_<type>), arguments are not.
# <type> can be discrete, continuous, etc. 
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = class), size = 3) + 
  scale_colour_brewer(name = 'Vehicle Class', palette = 'Set1') # Can title the legend easily this way.
...
ggplot(mpg) + 
  ...
  scale_colour_brewer(type = 'qual') +
  # scale_size(breaks = c(4,5,6,8)) <- This looks weird.
  scale_size_area(breaks = c(4,5,6,8), max_size = 4)
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy, colour = cty, size = cty)) # Colour palette becomes continuous.
...
ggplot(mpg) + 
  ...
  guides(color = "legend") # Color and size mapped to the same guide
...
ggplot(mpg) + 
  geom_point(aes(x = displ, y = hwy)) + 
  facet_grid(year ~ drv, scales = 'free_x')
